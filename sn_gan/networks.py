...
if __name__ == '__main__':
    g = SNDiscriminator(1, 64, [1, 1, 1, 1])
    #g = Generator(1, 64, 128, [2, 2, 2, 2])
    test = torch.randn(16, 1, 64, 64)
    # print("*" * 80)

    # for layer in g.net:
    #     if isinstance(layer, nn.Sequential) or isinstance(layer, DiscriminatorBlock):
    #         pass
    # for layer in g.modules():
    #     print(layer)
   
    # score, features = g(test)
    # print(score.shape)

    # for f in features:
    #     print(f.shape)
    print(g.get_model_size())
